...
```
    * iterating_variable can be any string
    * iteration_list is usually generated by a command
...
Let's see how these loops work with a few examples. First, create a new directory, ```~/CHE600/class04```, and ```cd``` into it.
...
```
Notice that semi-columns seperate the different parts of the loop.
