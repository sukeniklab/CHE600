...
As scientific knowledge grows, there is an ever-growin need to organize data in databases. These are built with clear logic, standardized formats, and can be used to search quickly and efficiently. Often, information from these database can be incorporated into your research. 

1. Often, we will need to look for a single entry in a database. For this, an easy option is simply to use the database's search function and pull up the relevant entry. However, sometimes we need to pull up dozens, hundreds, or even thousands of entries from a database. In cases where multiple entries must be pulled, it is often impossible to do so manually. Some databases have advanced search modes, but even those can be problematic.

2. In addition, often displaying the results in a webpage is often not efficient - we might need to click some links manually, or scroll to the bottom of the page to find the information we need. This is especially true if we need to look up thousands of entries.

3. For these cases, some databases offer a download of the entire database. This is not always available, and can often be problematic, both in terms of the download size and the requirement to search through a massive database on your local computer.

4. In many cases, the best way to interface with online databases is through programmatic access. That means we will write code to look up entries in the database.

5. Programmatic lookup allows us both to look up multiple entries, and also to format the output in a way that will be useful to us.

6. Today we will demonstrate how to do this using the [PubChem](https://pubchem.ncbi.nlm.nih.gov/) database. PubChem is an NIH-run database that is the world's largest collection of freely accessible chemical information. You can search chemicals by name, molecular formula, structure, and other identifiers, and find chemical and physical properties, biological activities, safety and toxicity information, patents, literature citations and more.
...
4. Not in the entry the accession code - P43220. This is an accession code for proteins that originates from another database called Uniprot (more about this one later.)
...
<img src="./images/PubChem_REST.png" width=450>
...
3. The output should be a CSV file. The final REST URL looks like this. Note that this URL is very versatile! You can now replace the protein accession P43220 with another protein accession code to pull that protein's list of binding molecules!
...
4. Let's now open this csv using pandas in a new notebook.
...
5. The columns now represent the full list of binding compounds. It includes binding constants and the result of binding (agonst/antagonist/etc.). It also includes specific identifiers:
...
2. Let's first compile all the cid's in our dataset, and format them as a string with commas separating each value:

```python
cids = df['cid'][:100].astype('str').str.cat(sep=',')
print(cids)
```

3. Now let's use this string as part of a REST lookup:
...
4. the variable ```r``` is a JSON file (JavaScript Object Notation) - this is a way to organize heirerchical data, but it is difficult to read. Here I've written a script that will export some selected information for each compound and place it in a new dataframe, ```df```.
...
5. Your task now is to use the ```pd.merge()``` function of pandas (we've seen this one before!) to combine the GLP-1 binding affinity with the molecular characteristics we've found here. 

6. Plot this binding affinity _vs_ the logP of all compounds and the binding affinity _vs_ the molecular weight of all compounds..

6. Upload your notebook to the upload link on Blackboard.
